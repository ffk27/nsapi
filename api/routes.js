...
function getDrukte(req, res) {
    ...
    Drukte.findOne({tijd: tijd, ritNummer: rit}, function (err, drukte) {
        if (err) {
            return res.json(err);
        }
        if (drukte) {
        ...
        request({url: 'http://www.ns.nl/reisinfo-api/service/treindrukte', qs: req.query}, function (error, response, body) {
            if (error) {
...
function getTreininfo(req, res) {
    ...
    Treininfo.findOne({vertrektijd: qs.vertrekstation, ritnummer: qs.q}).populate('info.materieeldelen.mat').exec((err, inf) => {
        if (err) res.json(err);
        if (inf) res.json(inf);
        ...
        request({url: url, qs: qs}, function (error, response, body) {
            ...
            try {
                const treininfo = new Treininfo({info: []});
                const mattypes = [];
                ...
                Materieel.find({mattype: {$in: mattypes}}, function (err, mats) {
                    ...
                    treininfo.info.forEach(info => {
                        info.materieeldelen && info.materieeldelen.forEach((matdeel, key) => {
                            ...
                            console.log(info.materieeldelen[key])
                        });
                    });
                    ...
                    Materieel.insertMany(matToAdd, function (err, addedMats) {
                        ...
                        treininfo.info.forEach(info => {
                            info.materieeldelen && info.materieeldelen.forEach((matdeel, key) => {
                                if (!matdeel.mat) {
                                    const addedMat = addedMats.find(mat => { return mat.mattype === matdeel.type });
...
function getStations(req, res) {
    request({url: 'http://www.ns.nl/reisinfo-api/service/stations/', qs: req.query}, function (error, response, body) {
        if (error) {
            return res.json(error);
        }
        try {
...
function getDepartures(req, res) {
    request({url: 'https://www.rijdendetreinen.nl/ajax/departures', qs: req.query}, function (error, response, body) {
        if (error) {
            return res.json(error);
        }
        try {
